...
resources_factor =   '
  ...
  mng =~ mng_1 + mng_2 
  inf =~ inf_1 + inf_2 + inf_3 + inf_4 + inf_5 + inf_6
  soc =~ soc_1 + soc_2 + soc_3 + soc_4 + soc_5 + soc_6
  sec =~ sec_1 + sec_2 + sec_3 + sec_4
...
```{r}
cooks_d <- genCookDist(jdr_measurement_fit_test, data = processed, check.lv.names = FALSE)

outlier_df <- processed %>%
  mutate(
    id = seq_len(nrow(.)),
    cooks_d = cooks_d,
    is_outlier_cooks = cooks_d > 10   # or 0.5 depending on preference
  )
