...
pwrsem_res_variances <- as.data.frame(inspect(jdr_measurement_fit, "std.all")$lambda)
...
out <- sim(
  nRep = 100,
  model = jdr_measurement_fit,          # model to fit in each replication
  generate = popModel,  # the population model (from pilot)
  n = 400     # same N as your pilot
)
summary(out)
