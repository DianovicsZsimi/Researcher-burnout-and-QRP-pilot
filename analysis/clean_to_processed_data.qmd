...
pp_stress_subscale = pp_data |> 
  ...
  mutate(
    validity = case_when(
      # Rule 1: if participant did not fill out questionnaire: valid
      all(is.na(c_across(starts_with("pps")))) ~ "valid",
      
      # Rule 2: if all items are identical: invalid
      n_distinct(na.omit(c_across(starts_with("pps")))) == 1 ~ "invalid",
      
      # Rule 3: otherwise valid
      TRUE ~ "valid"
    )
  ) %>%
  ungroup()
...
pp_attitude_subscale = pp_data |> 
  ...
  mutate(
    validity = case_when(
      all(is.na(c_across(starts_with("ppa")))) ~ "valid",
      n_distinct(na.omit(c_across(starts_with("ppa")))) == 1 ~ "invalid",
      TRUE ~ "valid"
    )
  ) %>%
  ungroup()
...
pp_resources_subscale = pp_data |> 
  ...
  mutate(
    validity = case_when(
      all(is.na(c_across(starts_with("ppr")))) ~ "valid",
      n_distinct(na.omit(c_across(starts_with("ppr")))) == 1 ~ "invalid",
      TRUE ~ "valid"
    )
  ) %>%
  ungroup()
...
jis_data = jis_data %>% 
  ...
  mutate(
    validity = case_when(
      all(is.na(c_across(starts_with("sec")))) ~ "valid",
      n_distinct(na.omit(c_across(starts_with("sec")))) == 1 ~ "invalid",
      TRUE ~ "valid"
    )
  ) %>%
  ungroup()
...
wlb_data = wlb_data %>% 
  ...
  mutate(
    validity = case_when(
      all(is.na(c_across(starts_with("wlb")))) ~ "valid",
      n_distinct(na.omit(c_across(starts_with("wlb")))) == 1 ~ "invalid",
      TRUE ~ "valid"
    )
  ) %>%
  ungroup()
...
workload_data = workload_data %>% 
  ...
  mutate(
    validity = case_when(
      all(is.na(c_across(starts_with("wld")))) ~ "valid",
      n_distinct(na.omit(c_across(starts_with("wld")))) == 1 ~ "invalid",
      TRUE ~ "valid"
    )
  ) %>%
  ungroup()
...
soc_sup_supervisor_data = soc_support_data |> 
  dplyr::select(soc_1, soc_2, soc_3, anonym_id) |> 
  rename_with(~str_replace(., "soc", "scs"))

soc_sup_colleague_data = soc_support_data |>
  dplyr::select(soc_4, soc_5, soc_6, anonym_id) |> 
  rename_with(~str_replace(., "soc", "scc")) %>% 
  rename(scc_1 = scc_4,
         scc_2 = scc_5,
         scc_3 = scc_6)
...
role_stressor_data = role_stressor_data %>% 
  ...
  mutate(
    validity = case_when(
      all(is.na(c_across(starts_with("rl")))) ~ "valid",
      n_distinct(na.omit(c_across(starts_with("rl")))) == 1 ~ "invalid",
      TRUE ~ "valid"
    )
  ) %>%
  ungroup()
...
survey_data = burnout_data |> 
  ...
  left_join(soc_sup_supervisor_data, by = "anonym_id") |>
  left_join(soc_sup_colleague_data, by = "anonym_id") |> 
...
```{r}
validity_cols <- grep("valid", names(survey_data), value = TRUE)

survey_data <- survey_data %>%
  rowwise() %>%
  mutate(
    overall_validity = if_else(
      any(c_across(all_of(validity_cols)) == "invalid"),
      "invalid",
      "valid"
    )
  ) %>%
...
```{r}
processed_data = survey_data_clean |> 
